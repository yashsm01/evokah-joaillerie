...
const { Collection, ProductType, Category, Style, Shape, Metal, PriceGroup, Role } = require('../models');
...
async function seed() {
  ...
  // ── Collections ─────────────────────────────────────────────
  const [engagement] = await Collection.findOrCreate({ where: { slug: 'engagement' }, defaults: { name: 'Engagement', sortOrder: 1 } });
  const [wedding]    = await Collection.findOrCreate({ where: { slug: 'wedding' },    defaults: { name: 'Wedding',    sortOrder: 2 } });
  await Collection.findOrCreate({ where: { slug: 'all' }, defaults: { name: 'Shop All', sortOrder: 3 } });
  console.log('  ✅ Collections');

  // ── Product Types ───────────────────────────────────────────
  for (const [name, slug] of [['Ring','ring'],['Necklace','necklace'],['Earring','earring'],['Bracelet','bracelet']]) {
    await ProductType.findOrCreate({ where: { slug }, defaults: { name } });
  }
  ...
  for (const [name, slug] of engCategories) {
    await Category.findOrCreate({ where: { slug }, defaults: { name, collectionId: engagement.id } });
  }
  const wedCategories = [['Wedding Band','wedding-band'],['Eternity Band','eternity-band']];
  for (const [name, slug] of wedCategories) {
    await Category.findOrCreate({ where: { slug }, defaults: { name, collectionId: wedding.id } });
  }
  ...
  for (const [name, slug] of styles) {
    await Style.findOrCreate({ where: { slug }, defaults: { name } });
  }
  console.log('  ✅ Styles');
  ...
  for (const name of shapes) {
    await Shape.findOrCreate({ where: { slug: name.toLowerCase() }, defaults: { name } });
  }
  console.log('  ✅ Shapes');
  ...
  for (const m of metals) {
    await Metal.findOrCreate({ where: { code: m.code }, defaults: m });
  }
  console.log('  ✅ Metals');
  ...
  for (const pg of priceGroups) {
    await PriceGroup.findOrCreate({ where: { slug: pg.slug }, defaults: pg });
  }
